...
items_db = {
    ...
    "T사 수사관 배지": {"cost": 250, "desc": "치명적인 위기의 순간, 1회 시간을 정지시켜 회피한 뒤 칼리의 시간을 조금 되돌려 기세를 꺾습니다."},
        ...
        for hour in range(1, target_hours + 1):
                ...
                if max_dice > 0:
                    ...
                    if roll == max_dice:
                        if guard == "LCD 에즈라":
                            ...
                            hour_log += "> 🌿 **[취수낭랑 - 성]** 싱클레어의 맑은 일격이 붉은안개의 참격을 유려하게 흘려냅니다!\n\n"
                        elif guard == "제2권속 산초":
            ...
            if current_team_power >= effective_kali_attack:
                ...
                if "중지 아비 마티아스" in selected_guards and available_sacrifices:
                    ...
                    hour_log += f"> ⏱️ **[시간 역행]** 방어선이 붕괴된 순간, T사 배지를 부수어 붉은안개가 기세를 올리기 전으로 시간을 되감습니다!\n\n"
                    hour_log += f"> ⚠️ **(강제 생존 / 붉은안개의 영구 위력 30 감소)**\n\n"
                elif revives_left > 0:
                    revives_left -= 1
